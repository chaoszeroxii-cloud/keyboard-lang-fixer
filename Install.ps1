<#
  One-shot installer. Copies the tool into the user's own app-data folder, sets
  it to start at login, and launches it -- no admin rights, no registry, nothing
  outside the user's profile.

  Normally launched by double-clicking Install.cmd. Run directly with:
      powershell -ExecutionPolicy Bypass -File Install.ps1
      powershell -ExecutionPolicy Bypass -File Install.ps1 -Uninstall
#>
[CmdletBinding()]
param(
    [switch]$Uninstall,
    # Skip the finishing message box (used by the tests).
    [switch]$Quiet
)

$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName System.Windows.Forms

$AppName     = 'Keyboard Language Fixer'
$source      = $PSScriptRoot
$target      = Join-Path $env:LOCALAPPDATA 'KeyboardLangFixer'
$startupLink = Join-Path ([Environment]::GetFolderPath('Startup')) "$AppName.lnk"

# Files that make up the tool. settings.json is deliberately absent: it belongs
# to the installed copy and must not be overwritten by a reinstall.
$payload = @(
    'KeyboardLangFixer.ps1'
    'Start-Hidden.vbs'
    'Start-Visible.cmd'
    'Install-Startup.ps1'
    'Install.ps1'
    'Install.cmd'
    'Uninstall.cmd'
    'Settings.cmd'
    'icon.ico'
    'README.md'
)

function Say([string]$text, [string]$colour = 'Gray') { Write-Host $text -ForegroundColor $colour }

function Stop-RunningCopy {
    $running = @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" |
        Where-Object { $_.CommandLine -like '*-File *KeyboardLangFixer.ps1*' -and
                       $_.CommandLine -notlike '*Get-CimInstance*' })
    foreach ($p in $running) {
        Say "  stopping running copy (PID $($p.ProcessId))"
        try { Stop-Process -Id $p.ProcessId -Force } catch { }
    }
    if ($running.Count) { Start-Sleep -Milliseconds 800 }
    return $running.Count
}

# ------------------------------------------------------------------------------
if ($Uninstall) {
    Say "Removing $AppName ..." 'Cyan'
    [void](Stop-RunningCopy)

    if (Test-Path -LiteralPath $startupLink) {
        Remove-Item -LiteralPath $startupLink -Force
        Say '  removed the startup shortcut'
    }

    $message = "$AppName has been stopped and will no longer start with Windows."
    # Only delete the installed copy, never the folder the installer was run from.
    if ((Test-Path -LiteralPath $target) -and ($source -ne $target)) {
        Remove-Item -LiteralPath $target -Recurse -Force
        Say "  deleted $target"
        $message += "`n`nDeleted:`n$target"
    } elseif ($source -eq $target) {
        $message += "`n`nThe program folder was left in place because the uninstaller is running from it:`n$target"
    }

    Say 'Done.' 'Green'
    if (-not $Quiet) {
        [System.Windows.Forms.MessageBox]::Show($message, $AppName, 'OK', 'Information') | Out-Null
    }
    exit 0
}

# ------------------------------------------------------------------------------
Say "Installing $AppName ..." 'Cyan'

$missing = @($payload | Where-Object { -not (Test-Path -LiteralPath (Join-Path $source $_)) })
# Only the script itself is essential; the rest are conveniences.
if ($missing -contains 'KeyboardLangFixer.ps1') {
    throw "KeyboardLangFixer.ps1 is missing from $source - copy the whole folder, not just the installer."
}

[void](Stop-RunningCopy)

if ($source -eq $target) {
    Say "  already installed in $target, refreshing settings only"
} else {
    if (-not (Test-Path -LiteralPath $target)) {
        New-Item -ItemType Directory -Path $target -Force | Out-Null
    }
    foreach ($file in $payload) {
        $from = Join-Path $source $file
        if (Test-Path -LiteralPath $from) { Copy-Item -LiteralPath $from -Destination $target -Force }
    }
    $toolsFrom = Join-Path $source 'tools'
    if (Test-Path -LiteralPath $toolsFrom) {
        Copy-Item -LiteralPath $toolsFrom -Destination $target -Recurse -Force
    }
    Say "  copied to $target"
}

# The shortcut has to point at the installed copy, so let that copy make it.
$startupInstaller = Join-Path $target 'Install-Startup.ps1'
if (Test-Path -LiteralPath $startupInstaller) {
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $startupInstaller | Out-Null
    Say '  set to start with Windows'
} else {
    Say '  Install-Startup.ps1 missing; skipped auto-start' 'Yellow'
}

$launcher = Join-Path $target 'Start-Hidden.vbs'
if (Test-Path -LiteralPath $launcher) {
    Start-Process wscript.exe -ArgumentList "`"$launcher`""
    Start-Sleep -Seconds 3
    Say '  started'
}

Say 'Done.' 'Green'
if (-not $Quiet) {
    [System.Windows.Forms.MessageBox]::Show(
        "$AppName is installed and running." +
        "`n`nSelect text you typed in the wrong language and press Win+Space." +
        "`n`nIt starts automatically when you log in. The tray icon has a" +
        " 'Change hotkey...' option, and Uninstall.cmd removes it." +
        "`n`nInstalled in:`n$target",
        $AppName, 'OK', 'Information') | Out-Null
}
exit 0
