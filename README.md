...
**งานหนักทำบน worker thread ของตัวเอง ไม่ใช่บน message loop** เพราะ low-level keyboard hook ถูก dispatch บน thread เดียวกับ message loop นั้น การแปลงหนึ่งครั้งใช้เวลาเกือบวินาที (รอ clipboard, รอ flyout, รอแอปปลายทาง) ถ้าทำบน thread นั้น hook จะตอบไม่ได้ตลอดช่วงนั้น → **ทุกปุ่มที่ผู้ใช้กดจะค้างรอ hook จนครบ `LowLevelHooksTimeout` (ดีฟอลต์ 300 ms ต่อปุ่ม)** ตัวอักษรจึงมาช้า สลับลำดับ และไปปนกับคีย์ที่โปรแกรมยิงเอง — อาการ "เปลี่ยนภาษาแล้วพิมพ์เร็ว ๆ แล้วรวน" มาจากตรงนี้ ย้ายออกไป worker แล้ว message loop ตอบ hook ได้ในหลักไมโครวินาที (ยัง re-seat hook หลังแปลงเสร็จไว้เป็นกันเหนียว)
...
หลัง paste ต้อง**ค้างข้อความที่แปลงไว้บน clipboard ต่ออีก ~0.7 วิ** ก่อนคืนของเดิม เพราะ Ctrl+V ไม่ได้ paste ทันที มันแค่เข้าคิว แอปจะไปอ่าน clipboard ตอนที่มันประมวลผลคีย์นั้น ซึ่งถ้าคิว input แน่น (เช่นผู้ใช้พิมพ์รัว) จะช้ากว่าที่ยิงไปมาก — คืนเร็วไปเมื่อไหร่ แอปจะไปอ่านเจอ**ของเก่าที่เพิ่งคืนกลับ** แล้ววางอันนั้นแทน
...
```powershell
# ตารางแปลง + Caps Lock + สลับเคส + ตรรกะ Smart Selection (ไม่แตะหน้าจอ, 161 checks)
