...
>
> **หมายเหตุ:** ทุกครั้งที่กดสองปุ่มนี้ โปรแกรมจะตรวจ selection ด้วย `Ctrl+C` (ใช้ `Ctrl+Insert` ใน console) เมื่อมีข้อความจะทำต่อทันที ส่วนกรณีไม่มี selection อาจรอหมดเวลาประมาณ 0.3–0.6 วินาที ถ้าไม่ต้องการให้ทำงานในบางโปรแกรม ให้ใส่ชื่อไว้ใน [ignore-list](#ignore-list--เว้นบางโปรแกรมไปเลย)
...
hook callback ต้องคืนค่าเร็วมาก ไม่งั้น Windows จะเลิกเรียก — จึงแค่ `PostMessage` ไปหาหน้าต่างของตัวเองแล้วคืนทันที

**งานแปลงทำบน STA worker ประจำที่มี message loop ของตัวเอง** ส่วน keyboard hook อยู่บน UI thread จึงยังตอบคีย์ได้ระหว่างรอ clipboard การแปลง การคืน clipboard และ spell checker ใช้ worker เดิมตลอด ไม่สร้าง thread ใหม่ทุกครั้ง และไม่ต้องข้าม COM apartment เพื่อเช็คคำ

clipboard ใช้ `WM_CLIPBOARDUPDATE` ปลุก worker ผ่าน event แทนการวน `Sleep(1)` ระหว่างรอ copy โดยยังตรวจ sequence number เพื่อกันการแจ้งเตือนเก่า หากลงทะเบียน listener ไม่สำเร็จ หรือข้อมูลยังอ่านไม่ได้ จะใช้การรอแบบจำกัดเวลาเป็นตัวสำรอง

ระหว่างที่แปลงอยู่ hook นับคีย์ที่ผู้ใช้พิมพ์จริงไว้ด้วย ถ้านับได้เพิ่มแปลว่า**เอกสารข้างล่างเปลี่ยนไปแล้ว** ทุกขั้นที่จะเขียนทับเอกสารจะเช็คก่อนแล้วยอมแพ้ ดีกว่าไปวางทับสิ่งที่เพิ่งพิมพ์
...
ถ้าสิ่งที่ก๊อปกลับมา**ลงท้ายด้วยขึ้นบรรทัดใหม่ ถือว่าไม่มี selection** — VS Code / Visual Studio / Notepad++ / IDE ค่าย JetBrains ทั้งหมดจะก๊อป**ทั้งบรรทัด**เมื่อกด Ctrl+C ตอนไม่ได้เลือกอะไร ถ้าเชื่อตามนั้นแล้วแปลงต่อ ตอน Ctrl+V จะไม่มี selection ให้ทับ = **แทรกทั้งบรรทัดที่แปลงแล้วลงไปตรงเคอร์เซอร์** (อาการ "เปลี่ยนภาษาเฉย ๆ แล้วมันวางอะไรยาว ๆ ให้")

หลัง paste ยังคงข้อความที่แปลงไว้บน clipboard ต่ออีก **700 ms** ให้แอปที่อ่านช้าทำงานได้ แต่คืน clipboard ในเบื้องหลัง จึงรับปุ่มครั้งถัดไปได้ทันที การกดซ้ำจะสืบทอด clipboard ต้นฉบับ และตรวจ sequence number ก่อนคืน เพื่อไม่ทับสิ่งที่ผู้ใช้เพิ่ง copy ใหม่ แม้ข้อความนั้นจะเหมือนข้อความที่แปลงก็ตาม

จับ sequence number **ก่อนส่ง Ctrl+V** และไม่เปิด clipboard อ่านซ้ำหลังส่ง เพราะแม้การเปิดอ่านชั่วครู่ก็อาจขวางการ paste ของแอปปลายทางจน selection กลายเป็นค่าว่างได้ การคืน clipboard จะเข้าคิว worker เดิม และงานคืนเก่าจะถูกยกเลิกเมื่อเริ่มการแปลงครั้งใหม่

สลับ input language ของหน้าต่างเดิมด้วย `WM_INPUTLANGCHANGEREQUEST` และรอจน Windows รายงานภาษาที่ต้องการ โดยไม่หน่วงคงที่หลังวาง ข้อความ Unicode บน clipboard ใช้ Win32 โดยตรงเพื่อลดการข้าม OLE apartment ของ worker thread
...
- **`Win+Space` กับ `Caps Lock` ต้องเช็คทุกครั้งว่ามีการลากคลุมมั้ย** — ส่งคำสั่ง copy ต่อการกด ถ้าไม่มี selection จะรอหมดเวลาประมาณ 0.3–0.6 วิ ในเกมหรือแอปที่ผูกปุ่ม copy ไว้ ให้ใส่ชื่อใน `IgnoreApps` หรือปิดปุ่มนั้นจาก tray
- **สลับเคสทำได้เฉพาะภาษาที่มีตัวพิมพ์เล็ก/ใหญ่** — ไทย/จีน/ญี่ปุ่นไม่มีเคส กด Caps Lock ที่คลุมไว้จะไม่เกิดอะไร (มีเสียงเตือน)
- **แอปที่ไม่รับ Ctrl+C**: มีตัวสำรองเป็น Ctrl+Insert ส่วน console ใช้ Ctrl+Insert เท่านั้น
...
ผลวัดบนเครื่องทดสอบวันที่ 23 กันยายน 2026 (นับหลังปล่อยปุ่ม ไม่รวมเวลาที่กดค้าง):

| กรณี | ก่อนแก้ | หลังแก้ |
|---|---:|---:|
| WinForms: พร้อมรับการกดครั้งถัดไป | 875–1,178 ms | 58–128 ms |
| WinForms: ข้อความที่ครอบเปลี่ยนจริง | 163–459 ms | 54–125 ms |
| Chrome: textarea และ contenteditable, 12 ครั้ง | ไม่ได้วัด | 26–91 ms |

ตัวเลขเป็นผลทดสอบบนเครื่องนี้ ไม่ใช่การรับประกันเวลาของทุกแอป ชุดทดสอบ latency ตรวจการกดซ้ำ การวางช้า 400 ms และการรักษา clipboard ด้วย ส่วน Smart Selection ที่ไม่ได้ครอบข้อความยังต้องรอตรวจ selection และส่งปุ่มลูกศรตามจำนวนตัวอักษร

รอบปรับ worker/event เพิ่มเติมในวันเดียวกัน: WinForms ก่อนแก้รอบนี้ 15 ครั้งเฉลี่ย **103.1 ms** หลังแก้ 30 ครั้งเฉลี่ย **71.1 ms** (ช่วง **52–103 ms**) นับจากปล่อยปุ่มจนข้อความเปลี่ยน พร้อมรับครั้งถัดไปเฉลี่ยจาก **108.5 เป็น 76.6 ms** การวัดสองชุดทำตามลำดับบนเครื่องเดียวกัน จึงยังมีความแปรปรวนจากโหลดเครื่อง
